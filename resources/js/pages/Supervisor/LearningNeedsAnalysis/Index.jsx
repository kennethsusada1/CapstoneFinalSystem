...
function ReviewForm({ entry, supervisorName }) {
    const submittedSkills = entry.skill_assessments ?? {};
    const skills = skillGroups.flatMap((group) => group.items).filter((skill) => Object.prototype.hasOwnProperty.call(submittedSkills, skill));
            ...
            <div className="lna-review-wide">
                ...
                <div className="lna-supervisor-ratings">
                    {skillGroups.map((group) => {
                        const groupSkills = group.items.filter((skill) => skills.includes(skill));

                        if (groupSkills.length === 0) return null;

                        return (
                            <section className="lna-supervisor-group" key={group.title}>
                                <h3>{group.title}</h3>
                                <div className="lna-supervisor-group-list">
