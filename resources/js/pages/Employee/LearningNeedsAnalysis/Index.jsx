...
export default function Index({ lnaEntries, recommendations }) {
    ...
    return <AppLayout title="LNA Assessment" description="Employee / Learning Needs Analysis"><div className="emp-page">
        <PageHero kicker="CAPABILITY CHECK-IN" title="Identify where growth will make the biggest difference." description="Describe your current competency gap and preferred intervention. The initial prescriptive and predictive analytics remain static and transparent while the workflow is being validated." href="/employee/recommendations" action="View recommendations" icon="bi-lightbulb" />
        ...
        <section className="emp-grid-2">
            ...
            <Panel title="Generated Analytics" subtitle="Static prescriptive and predictive outputs"><div className="emp-list">{(recommendations ?? []).length === 0 && <EmptyState icon="bi-stars" title="Analytics awaiting input" text="Submit an assessment to generate your initial skills-gap prescription." />}{(recommendations ?? []).map((item) => <div className="emp-item" key={`${item.lna_id}-${item.predicted_training_recommendation}`}><div className="emp-row"><div className="emp-title">{item.focus_area}</div><StatusPill tone={tone(item.priority_level)}>{item.priority_level} priority</StatusPill></div><div className="emp-muted" style={{ marginTop: '.55rem' }}>PRESCRIPTIVE ANALYTICS</div><div className="emp-copy">{item.prescribed_skills_gap}</div><div className="emp-muted" style={{ marginTop: '.55rem' }}>PREDICTIVE ANALYTICS</div><div className="emp-title">{item.predicted_training_recommendation}</div></div>)}</div></Panel>
        </section>
        <Panel title="Assessment History" subtitle="Submitted records and supervisor review status"><div className="emp-table-wrap"><table className="emp-table"><thead><tr><th>Focus Area</th><th>Competency Gap</th><th>Priority</th><th>Submitted</th><th>Status</th></tr></thead><tbody>{(lnaEntries ?? []).map((item) => <tr key={item.id}><td><strong>{item.focus_area}</strong></td><td>{item.competency_gap}</td><td><StatusPill tone={tone(item.priority_level)}>{item.priority_level}</StatusPill></td><td>{item.submitted_on || 'Draft'}</td><td><StatusPill tone={item.status === 'approved' ? 'success' : 'info'}>{item.status}</StatusPill></td></tr>)}</tbody></table>{(lnaEntries ?? []).length === 0 && <EmptyState title="No LNA records" text="Your submitted assessments will be listed here." />}</div></Panel>
