...
export default function Dashboard({ stats, progressCards, charts, recentTrainings, highlights, recommendations, notifications }) {
    ...
    return <AppLayout title="Employee Dashboard" description="Employee / Learning Journey"><div className="emp-page">
        ...
        <section className="emp-grid-2">
            <Panel title="Recommended Trainings" subtitle="Based on your current skills gaps" action={<Link className="emp-link" href="/employee/recommendations">View all</Link>}><div className="emp-list">{(recommendations ?? []).length === 0 && <EmptyState icon="bi-lightbulb" title="No recommendations yet" text="Complete an LNA assessment to generate recommendations." />}{(recommendations ?? []).slice(0, 4).map((item, index) => <div className="emp-item" key={`${item.predicted_training_recommendation}-${index}`}><div className="emp-title">{item.predicted_training_recommendation}</div><div className="emp-muted" style={{ marginTop: '.25rem' }}>{item.focus_area}</div><div className="emp-copy" style={{ marginTop: '.4rem' }}>{item.prescribed_skills_gap}</div></div>)}</div></Panel>
